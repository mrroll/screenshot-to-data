...
import { screenshotQueryArgsSchema } from '@/graphql/schema/Objects/Screenshot/queries/screenshot/args';
  ...
  if (screenshot.S3URL === null) {
    ...
    const params = z
      .object({
        ...
        'X-Amz-Expires': z.coerce
          .number()
          .transform((value) => Duration.fromObject({ seconds: value })),
      })
      .parse(S3URLSearchParams);

    const expiration = params['X-Amz-Date'].plus(params['X-Amz-Expires']);
